...
[video/](./video) contains a 30-second motion graphic built from the site's
...
(puppeteer-core) and assembles `neo-compose-30s.mp4` with ffmpeg-static,
including a subtle generated ambient pad.

```bash
cd video
npm install
node render.mjs   # → video/neo-compose-30s.mp4
```
